...
export function promptUnderstandFromText(articleText: string): Message[] {
  return withPromptSetting('promptUnderstandFromText', [
    {
      role: 'system',
...
export function promptUnderstandFromOutline(outlineText: string): Message[] {
  return withPromptSetting('promptUnderstandFromOutline', [
    {
      role: 'system',
...
export function promptChatFollowup(
  ...
  return withPromptSetting('promptChatFollowup', [
    {
      role: 'system',
...
export function promptExtractStyle(articleText: string): Message[] {
  return withPromptSetting('promptExtractStyle', [
    {
      role: 'system',
...
export function promptExtractStyleProfile(articleText: string): Message[] {
  return withPromptSetting('promptExtractStyleProfile', [
    {
      role: 'system',
...
export function promptExtractCases(
  ...
  return withPromptSetting('promptExtractCases', [
    {
      role: 'system',
...
export function promptExtractBackgroundMaterial(
  ...
  return withPromptSetting('promptExtractBackgroundMaterial', [
    {
      role: 'system',
...
export function promptWriteSection(
  ...
  return withPromptSetting('promptWriteSection', [
    {
      role: 'system',
...
export function promptReviseSection(
  ...
  return withPromptSetting('promptReviseSection', [
    {
      role: 'system',
...
export function promptRewriteSelection(
  ...
  return withPromptSetting('promptRewriteSelection', [
    {
      role: 'system',
...
export function promptFinishDraft(
  ...
  return withPromptSetting('promptFinishDraft', [
    {
      role: 'system',
...
export function promptAdjustFinish(
  ...
  return withPromptSetting('promptAdjustFinish', [
    {
      role: 'system',
...
- 如果论文背景或 Stage1 研究方法建议中出现问卷、量表、访谈、KANO、AHP、熵权法、回归、中介、信效度、KMO、Bartlett、T检验、ANOVA、EFA、相关分析、扎根/开放/主轴编码、情感编码、主题提取、案例评价、用户研究、数据分析等内容，大纲必须预留研究结果承载位。
- 流程上仍是先确认大纲、再生成正文，研究计算放在全文初稿之后补入；大纲阶段不生成任何具体数值，只设计能承接表/图/结果的章节位置。
- 对实证/量化/中介/信效度/设计评价/质性编码类论文，「研究设计与数据来源」和「数据分析与研究结果」必须各自作为独立的一级章节（独立“一、二、三…”级，不能塞成某章的子小节）；「结果讨论与优化建议」作为独立一级章节或紧随其后。
- 「研究设计与数据来源」章承接：研究对象、样本/材料、变量与操作化定义、量表/指标体系/问卷/访谈提纲/编码方案、数据收集与处理流程。
- 「数据分析与研究结果」章必须按论文实际所用方法拆出对应二级小节，且每个小节都要为对应的【统计表】和【图】预留承接位（正文用“如表X-X所示”“如图X-X所示”的句式预留，不填具体数字）：
  · 量化（问卷/量表/SEM/中介）：描述性统计（表＋均值图）／信度与效度检验（Cronbach α 表＋KMO·Bartlett 表）／相关分析（相关矩阵表＋热力图）／回归分析（系数表）／中介效应检验（路径系数与 Bootstrap 置信区间表＋中介路径图）／差异分析（T检验·ANOVA 表＋箱线图）——按题目实际涉及取舍。
  · 设计评价（KANO/AHP）：KANO 需求分类（Better-Worse 表＋堆叠图）／指标权重与一致性检验（AHP 权重表＋排序图）／综合优先级（优先级矩阵表）。
  · 质性（扎根/主题/情感编码）：开放与主轴编码（编码表）／主题归纳（主题频次表＋频次图或编码树）／情感编码（情感编码说明表）。
  · 混合研究：量化与质性两套小节都要预留。
- 「结果讨论与优化建议」章把上述结果转化为设计策略、优化路径、理论讨论或实践建议。`

  return withPromptSetting('promptGenerateOutline', [
    {
      role: 'system',
...
export function promptReviseOutline(
  ...
  return withPromptSetting('promptReviseOutline', [
    {
      role: 'system',
...
export function promptGeneratePaperPlan(
  ...
  return withPromptSetting('promptGeneratePaperPlan', [
    {
      role: 'system',
...
export function promptGenerateFrontMatter(
  ...
  return withPromptSetting('promptGenerateFrontMatter', [
    {
      role: 'system',
...
export function promptSummarizeGeneratedChapter(chapterTitle: string, chapterContent: string): Message[] {
  return withPromptSetting('promptSummarizeGeneratedChapter', [
    {
      role: 'system',
...
export function promptGenerateChapter(
  ...
  return withPromptSetting('promptGenerateChapter', [
    {
      role: 'system',
