...
export function buildResearchToolPrompt(
  ...
  const modeRules: Record<ResearchToolMode, string[]> = {
    ...
    ahp: [
      '必须建立目标层、准则层、指标层三级结构，指标层不少于8个指标。',
      '必须给出专家两两比较表设计、1-9标度说明、一致性检验CR标准和权重计算说明。',
      '指标必须来自当前论文对象，而不是通用管理学模板。',
    ],
    coding: [
...
export function validateResearchTool(mode: ResearchToolMode, text: string): ResearchToolQualityCheck {
  ...
  if (mode === 'ahp') {
    ...
    if (indicatorCount < 8) issues.push(`AHP指标偏少，当前识别约 ${indicatorCount} 个，建议不少于 8 个。`)
